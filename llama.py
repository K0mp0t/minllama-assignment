...
class RMSNorm(torch.nn.Module):
    ...
    def _norm(self, x):
        ...
        return x / torch.sqrt(torch.square(x).mean(-1) + self.eps).unsqueeze(-1)
...
class Attention(nn.Module):
    ...
    def compute_query_key_value_scores(self,
        ...
        # (B, H, L, D) @ (B, H, D, L) -> (B, H, L, L) -(softmax)> (B, H, L, L)
        attn_scores = torch.softmax(query @ key.permute((0, 1, 3, 2)) / math.sqrt(self.head_dim), dim=-1)
...
class LlamaLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        ffn_output = attn_output + self.feed_forward(self.ffn_norm(attn_output))
...
class Llama(LlamaPreTrainedModel):
    ...
    def generate(self, idx, max_new_tokens, temperature=1.0):
        ...
        for _ in range(max_new_tokens):
            ...
            if temperature == 0.0:
                idx_next = logits.argmax(dim=-1).unsqueeze(0)
            else:
                '''
